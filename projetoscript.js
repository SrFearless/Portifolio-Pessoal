...
let indiceAtual = 1; // Começa no primeiro texto

function mostrarConteudo(indice) {
  const totalConteudo = 6;

  // Oculta todos os textos e imagens
  for (let i = 1; i <= totalConteudo; i++) {
      document.getElementById(`texto${i}`).classList.add("invisivel");
      document.getElementById(`imagem${i}`).classList.add("invisivel");
  }

  // Exibe apenas o texto e imagem correspondentes ao índice clicado
  document.getElementById(`texto${indice}`).classList.remove("invisivel");
  document.getElementById(`imagem${indice}`).classList.remove("invisivel");
}